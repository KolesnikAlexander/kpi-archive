...
_DATA SEGMENT WORD PUBLIC 'DATA' use16
    ...
    int33set db 0           ; ознака перехоплення переривання від мики
    ...
    rbtn_pressed dw 0       ; ознака того, чи була нажата права
                            ; кнопка мишки(0-не була/0ffh - була)
	pointerOffset dw ?
	taskToSht dw ?
_DATA ENDS
...
;--------------------------------------------------------------------------
;  процедура "перехоплення" переривання від мишки (int33)
;--------------------------------------------------------------------------
setint33 proc
;--------------------------------------------------------------------------
    mov al, int33set
    ...
    jnz zero_33h
    mov ah, 35h             ; отримати вектор переривання
    mov al, 74h             ; переривання від мишки (74)
    int 21h                 ; значення що повертається:
                            ; es:bx - вказівник на системну процедуру
                            ; обробки переривання від мишки

    mov cs:int33ptr, bx     ; зберегти логічну адресу системної
    mov cs:int33ptr+2, es   ; процедури в сегменті кодів

    mov ax, 0               ; ініціалізувати мишку
    int 33h

    mov ax, 1               ; відобразити курсор
    int 33h

    mov dx, offset userint33
    push ds
    push cs                 ; ds:dx - вказівник на процедуру користувача
    pop ds                  ; оброб. переривання від мишки
...
;--------------------------------------------------------------------------
    ...
    mov dx, cs:int33ptr     ; ds:dx - покажчик на початкову (системну)
    mov ds, cs:int33ptr+2   ; процедуру обробки переривання від мишки

    mov ah, 25h             ; встановити вектор системної процедури
    mov al, 74h             ; обробки переривання від мишки
    int 21h

    mov ax, 2               ; сховати курсор
    int 33h
...
;----------------------------------------------------------------------------
; Процедура обробки переривань від мишки,
; викликається при натисненні на праву клавішу мишки,
; знімає всі задачі, крім задачі зі станом execute та фонової
;----------------------------------------------------------------------------
userint33 proc far
;----------------------------------------------------------------------------


    btn_right equ 11b       ; код натиснутої правої кнопки
    pushad
    pushf

    call cs:dword ptr int33ptr

    mov ax, 3               ; отримати поточну позицію курсора
    int 33h                 ; та статус кнопок
    ...
    shr cx, 3               ; перетворення пікселів в координату симовла
    shr dx, 3               ; (один символ має розмір 8х8 в текстовому режимі)
	
	
	
	;;;;;;;;;;;;;;;;;;;;;;;;;
	;вычисление смещеня курсора от начала экрана
	...
	end_loop2:
	add ax,CX
	add ax,CX ; завершення обчислення
	mov pointerOffset, ax
	
	
	
	
	mov bx, pointerOffset
	mov ecx, 0
	while_beg:
		mov ax, screen_addr[ecx*2]
		cmp bx, ax
		jl while_end
			inc ecx
		jge while_beg
	...
	push screen_addr[ecx*2]
    push 2946
    call show	

    mov rbtn_pressed, 0ffh
...
ui3325: 
    ...
    mov init[bx], absent    ; зняття задачі з виконання

    push cx                     ; заповнення нулями області відеопам'яті
    cld                         ; яку використовує задача.
    mov ah, 1010b               ; 1010b атрибут символу, атрибут фона - 0(чорний)
    mov al, '0'
    mov cx, 22                  ; 22 розряда у лічильника
    mov di, screen_addr[ebx*2]  ; координата лічильника
    add di, 6
    rep stosw
    pop cx
...
;SETINT
    ...
    call setint33               ; "перехоплення" int33
