...
mov esi,[ebp + 8] ;pointer to M1
mov edi,[ebp + 12] ;pointer to M2
mov edx,[ebp + 16] ;length

mov ecx, 0 ;index of elements

loop32:
	cmp ecx, edx
	jae beforeLoop8 ;all possible 32b checks were done

	mov ebx, [esi + ecx]
	cmp ebx, [edi + ecx] ;compare 4 bytes 
	jne cmpBytes32; if 1st and 2nd are not equal
...
loop8:
	...
	jz trueEnd
	mov bl, [esi + ecx]
	cmp bl, [edi + ecx] ;compare 1 byte
	jne cmpBytes8; if 1st and 2nd are not equal
	add ecx, 1
