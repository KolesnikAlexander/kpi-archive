...
public class Controller implements Initializable {
    ...
    public void uploadButtonClicked(MouseEvent event) {
        GamesDao.deleteAll();

        boolean stadiums = DimentionsDao.uploadStadiums();
        boolean clubs = DimentionsDao.uploadClubs();
        boolean championships = DimentionsDao.uploadChampionships();

        if(stadiums && clubs && championships){
            Alert alert = new Alert(Alert.AlertType.INFORMATION,"Successfully uploaded");
            alert.showAndWait();
            return;
        }
        ...
        if(!championships) {
            message+=" championships ";
        }
        Alert alert = new Alert(Alert.AlertType.ERROR,"Cannot upload"+message);
        alert.showAndWait();
    }

    public void onCallButtonClicked(MouseEvent event) {
        numOfGamesLabel.setText("Number of games: "+GamesDao.storedProcedure());
