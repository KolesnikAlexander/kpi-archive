 ...
 * якщо масимально допустима довжина черги не перевищена */
	sem_getvalue(&sem_q,&sem_value);                          //<---------- Никто не вклинеться? (Работает только если продюсер один) После вып. этой 
	if (sem_value < max_queue_length) {                       //<---------							операции элементов мож. становиться только меньше, а не больше

/* Захоплення м'ютекса черги для додавання нового елементу */
            pthread_mutex_lock (&mut_q);
																//<--------- Проверить sem_getvalue, продолжить либо разблокировать мьютекс и ждат
            add_elem();
